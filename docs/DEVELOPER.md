...
│       │   ├── i18n_language.py # key 模版（KEYS 权威清单 + 分组约定）
│       │   ├── i18n_en.py       # 英文 (EN) 字典
│       │   ├── i18n_zh.py       # 中文 (ZH) 字典
...
│       ├── core/                # 业务核心层
│       │   ├── manager.py       # SSHKeyManager 门面（组合服务 + 命令编排）
│       │   ├── errors.py        # 业务异常（SSHMError / ValidationError）
│       │   ├── keystore.py      # KeyStore 密钥文件服务
│       │   ├── config.py        # SSHConfigManager
│       │   ├── state.py         # StateManager 状态持久化
│       │   ├── backup.py        # BackupService 备份服务
│       │   ├── gitrepo.py       # GitRepoService URL / 别名 / hostname
│       │   ├── author.py        # AuthorService 作者服务
│       │   ├── sshtest.py       # SSHTester SSH 连接测试
│       │   ├── rewrite.py       # git 历史重写
│       │   ├── updater.py       # UpdateManager 更新服务
│       │   ├── path.py          # PATH 配置
│       │   └── commands/        # 命令编排层（按指令分包）
│       │       ├── keys.py      # KeyCommands：list/add/remove/switch/tag/rename
│       │       ├── repo.py      # RepoCommands：use/clone/info/test
│       │       ├── author.py    # AuthorCommands：author 系列 + auto-author
│       │       └── system.py    # SystemCommands：lang
├── scripts/
...
- **core 层**：纯业务逻辑，不依赖 CLI
  - 服务：`KeyStore` / `SSHConfigManager` / `StateManager` / `BackupService` / `GitRepoService` / `AuthorService` / `SSHTester` / `rewrite` / `errors`
  - 门面：`SSHKeyManager`（组合服务，公开 API 委托到服务与命令编排组）
  - 命令编排：`commands/`（keys / repo / author / system，按指令分包，每模块单一职责）
  - 系统功能：`updater.py`（更新）、`path.py`（PATH）
...
flowchart TB
    ...
    subgraph Core["核心业务层 (src/sshm/core/)"]
        ...
        Commands["commands/<br/>keys / repo / author / system"]
        Services["服务<br/>keystore / config / state / backup /<br/>gitrepo / author / sshtest / rewrite"]
        Updater["updater.py<br/>版本更新"]
        Path["path.py<br/>PATH 管理"]
    end
