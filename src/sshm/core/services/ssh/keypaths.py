...
def private_key_path(ssh_dir: Path, key_type: str, label: str | None = None) -> Path:
    """私钥文件路径：label 存在 → id_<type>.<label>，否则 id_<type>。"""
    name = f"id_{key_type}.{label}" if label else f"id_{key_type}"
    return ssh_dir / name


def public_key_path(ssh_dir: Path, key_type: str, label: str | None = None) -> Path:
    """公钥文件路径（= 私钥名 + '.pub'）。"""
    return Path(f"{private_key_path(ssh_dir, key_type, label)}.pub")


def key_paths(ssh_dir: Path, key_type: str, label: str | None = None) -> tuple[Path, Path]:
    """私钥 + 公钥文件路径。"""
