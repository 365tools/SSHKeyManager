...
from ...i18n import _
from ...ui.console import pad_cell
...
class RepoCommands:
    ...
    def use_key_for_repo(self, label: str, repo_path: Union[str, Path] = '.',
        ...
        try:
            ...
            parsed = self.m._parse_git_url(current_url)
            if not parsed:
            ...
            repo_hostname = self.m._resolve_repo_hostname(current_url)
            if not self.m._align_hostname_with_repo(label, repo_hostname, skip_confirm):
                return

            # 确保 SSH config 别名存在且唯一（此时 hostname 已对齐，别名正确）
            host_alias = self.m._get_host_alias(label)
            self.m._update_ssh_config_alias(label, key_file)
            ...
            test_ok, test_msg = self.m._test_ssh_connection(host_alias)
            if test_ok:
            ...
            self.m.author.apply_auto_author(label, repo_path=str(repo_path),
                                            scope='local')
    ...
    def clone_with_label(self, label: str, url: str,
        ...
        # 解析 URL
        parsed = self.m._parse_git_url(url)
        if not parsed:
            self.m._fail("❌ " + _("err.failed_parse"))
            return
        ...
        repo_hostname = self.m._resolve_repo_hostname(url)
        if not self.m._align_hostname_with_repo(label, repo_hostname, skip_confirm):
            return
        ...
        author = self.m._get_author_info(label, repo_path=cloned_dir,
                                         infer_from_remote=False)
        if author and (author.get('name') or author.get('email')):
            print()
            self.m.set_repo_author(label, cloned_dir, skip_confirm=True,
                                   infer_from_remote=False)
    ...
    def show_repo_info(self, repo_path: Union[str, Path] = '.'):
        ...
        try:
            ...
            parsed = self.m._parse_git_url(remote_url)
            if parsed:
                platform, user, repo = parsed
                print(f"\n📊 {_('lbl.parsed_info')}")
                print(f"  ├─ {_('lbl.platform')} {platform}")
                ...
                if match:
                    ...
                    label = self.m._resolve_label_from_alias(host_alias)
                    if label:
                        ...
                        if key_type:
                            ...
                            if ssh_config.exists():
                                ...
                                block = self.m._extract_ssh_config_block(host_alias)
                                if block:
                                    print(f"\n📝 {_('lbl.ssh_config')}")
                                    for line in block:
                                        print(f"  {line}")
        ...
        if test_all:
            ...
            if network_tasks:
                ...
                with ThreadPoolExecutor(max_workers=workers) as pool:
                    future_map = {
                        label: pool.submit(self.m._test_ssh_connection, alias)
            ...
            success, message = self.m._test_ssh_connection(host_alias)
            if success:
                print(f"✅ {message}")
            else:
