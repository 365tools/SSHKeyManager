...
class RepoCommands:
    ...
    def use(self, label: str, repo_path: Union[str, Path] = '.',
        ...
        try:
            ...
            self.m.author_service.apply_auto_author(label, repo_path=str(repo_path),
                                            scope='local')
    ...
    def clone(self, label: str, url: str,
        ...
        author = self.m.author_service.get_author_info(label, repo_path=cloned_dir,
                                                       infer_from_remote=False)
        if author and (author.get('name') or author.get('email')):
            print()
            self.m.author.use(label, cloned_dir, skip_confirm=True,
                              infer_from_remote=False)
