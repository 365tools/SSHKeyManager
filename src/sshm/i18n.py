...
ZH: Dict[str, str] = {
    ...
    # ---- clone ----
    "Clone with Key: {label}": "使用密钥克隆: {label}",
    "Source URL:": "源 URL:",
    "Clone URL:": "克隆 URL:",
    "Clone with this URL and configure the key?": "是否使用此 URL 克隆并配置该密钥？",
    "Clone failed: {err}": "克隆失败: {err}",
    "Clone complete!": "克隆完成！",
    "This repo now uses key: {label}": "该仓库现已使用密钥: {label}",
    "clone a Git repo using a specific key": "使用指定密钥克隆 Git 仓库",
    "key label to use for cloning": "用于克隆的密钥标签",
    "Git repo URL (e.g. git@github.com:user/repo.git)": "Git 仓库 URL（如 git@github.com:user/repo.git）",
    "target directory name (default: repo name)": "目标目录名（默认使用仓库名）",

    # ---- auto-author ----
    "view or toggle auto-switch author with key": "查看或切换凭据-人员自动联动开关",
    "'on' or 'off' to enable/disable, empty to show status": "输入 on/off 开启/关闭，留空显示当前状态",
    "show current status": "显示当前状态",
    "Auto-Switch Author with Key": "凭据-人员自动联动",
    "Auto-author switch: {status}": "自动联动开关: {status}",
    "When you switch a key, its bound author is auto-applied.": "切换凭据时，其绑定的人员会自动应用。",
    "Switch a key will NOT change author. Use 'sshm author use' manually.": "切换凭据不会改变人员，请手动使用 'sshm author use'。",
    "Usage: sshm auto-author on|off": "用法: sshm auto-author on|off",
    "Now switching a key will auto-apply its bound author.": "现在切换凭据将自动应用其绑定的人员。",
    "Now switching a key will NOT change author.": "现在切换凭据不会改变人员。",
    "Auto-set author for key {label}": "已为凭据 {label} 自动设置人员",
    "auto author switch failed: {err}": "自动切换人员失败: {err}",
    "on": "开启",
    "off": "关闭",

    # ---- author fix ----
    "rewrite author/email in all git history": "重写所有 Git 历史中的作者/邮箱",
    "old author name to match": "要匹配的旧作者名",
    "new author name to replace": "要替换成的新作者名",
    "old email to match": "要匹配的旧邮箱",
    "new email to replace": "要替换成的新邮箱",
    "skip confirmation (rewrite all matching history)": "跳过确认（重写所有匹配的历史）",
    "Rewrite Author History": "重写作者历史",
    "Current authors in history:": "历史中的现有作者:",
    "Will rewrite {count} commits.": "将重写 {count} 个提交。",
    "This rewrites git history (commit hashes change).": "此操作会重写 Git 历史（提交哈希会改变）。",
    "You must force-push afterwards: git push --force": "之后必须强制推送: git push --force",
    "Continue rewriting history?": "继续重写历史？",
    "Rewrite failed: {err}": "重写失败: {err}",
    "History rewritten!": "历史已重写！",
    "Matched commits: {count}": "匹配提交: {count}",
    "Rewritten lines: {count}": "重写行数: {count}",
    "Original refs are backed up under refs/original/.": "原始引用已备份到 refs/original/ 下。",
    "To publish, force-push all branches: git push --force --all": "发布时需强制推送全部分支: git push --force --all",
    "Specify --old-name or --old-email to match": "请指定 --old-name 或 --old-email 作为匹配条件",
    "Specify --new-name or --new-email to replace": "请指定 --new-name 或 --new-email 作为替换值",
    "No commits match the given old author/email. Nothing to rewrite.": "没有提交匹配给定的旧作者/邮箱，无需重写。",
