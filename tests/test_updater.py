...
class TestCache:
    ...
    def test_invalid_version_in_cache_rejected(self, updater):
        # 版本无法解析（< 0）应视为无效缓存
        updater.CACHE_FILE.write_text(
            json.dumps({'version': 'not-a-version',
                        'download_url': 'https://x/y.exe'}), encoding='utf-8')
        assert updater._get_cache() is None
...
class TestCheckUpdate:
    ...
    def test_uses_cache_when_fresh_and_newer(self, updater):
        """有有效缓存且版本较新时，不发起网络请求"""
        updater._save_cache(
            {'version': 'v99.0.0', 'download_url': 'https://x/y.exe', 'body': '', 'published_at': ''})
    ...
    def test_no_update_when_current_newer(self, updater):
        ...
        with patch('sshm.core.services.net.updater.urlopen',
                   return_value=self._fake_response({'tag_name': 'v1.0.0'})) as mock_open:
    ...
    def test_finds_platform_asset(self, updater):
        ...
        payload = {
            'tag_name': 'v2.0.0',
            'assets': [
                {'name': 'sshm-windows-x64.exe', 'browser_download_url': 'https://x/win.exe'},
