...
class TestRewriteMatchAll:
    ...
    def test_match_all_unifies_authors(self, tmp_path):
        ...
        repo = setup_repo_with_commits(tmp_path, [
            ('Alice', 'a@x.com'), ('Bob', 'b@x.com'), ('Carol', 'c@x.com')])
        r = rewrite_history(repo, RewriteConfig(
            match_all=True, new_name='Unified', new_email='u@x.com'))
        assert r['matched_commits'] > 0
...
class TestRewriteExcludesOriginalRefs:
    ...
    def test_authors_after_rewrite_ignores_backup(self, tmp_path):
        ...
        rewrite_history(repo, RewriteConfig(old_email='old@x.com',
                                            new_email='new@x.com'))
    ...
    def test_second_rewrite_no_false_positive(self, tmp_path):
        ...
        r = rewrite_history(repo, RewriteConfig(old_name='Alice',
                                                new_name='Carol'))
        assert r['matched_commits'] == 0
        assert r['rewritten'] == 0
