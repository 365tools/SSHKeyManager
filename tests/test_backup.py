...
def backup_env(tmp_path: Path):
    ...
    return {
        'ssh_dir': ssh_dir, 'backup_dir': backup_dir,
        'state_file': state_file, 'config_file': config_file,
    }
...
def service(backup_env):
    ...
    errors = []
    svc = BackupService(
        backup_env['ssh_dir'], backup_env['backup_dir'],
        backup_env['state_file'], backup_env['config_file'],
...
class TestRestoreBackup:
    ...
    def test_restore_key_type_filter(self, service, backup_env, tmp_path):
        ...
        (backup_env['ssh_dir'] / 'id_ed25519').unlink()
        # 只恢复 ed25519 类型
        service.restore(backup_path.name, key_type='ed25519',
                               skip_confirm=True)
        assert (backup_env['ssh_dir'] / 'id_ed25519').exists()
